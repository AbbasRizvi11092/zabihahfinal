...
import {ScrollView, Text, View,StyleSheet, Image,TextInput,Appearance, SafeAreaView,FlatList,TouchableOpacity} from 'react-native';
...
const restaurantlistview=({navigation})=> {
 ...
 const [res_keyword,setResKeyword] = useState()
  ...
  useEffect(()=>{
    Axios
    .post(ServiceConstant.record_around_location + "?" + "key" + "=" + ServiceConstant.key + "&" + "uuid" + "=" + ServiceConstant.uuid + "&" + "id" + "=" + ServiceConstant.user_login_data + "&" + "l" + "=" + ServiceConstant.user_lat + "," + ServiceConstant.user_long + "&" + "r" + "=" + 10 + "&" + "t" + "=" + 1)
    .then(function(data){
     setMarketNum(data.data.data)
     setRestaurantDetail(data.data.data.restaurants)
      // alert(JSON.stringify(data.data.data.restaurants[0]))
    })
  //   Axios
  // .post(ServiceConstant.record_around_location + "?" + "key" + "=" + ServiceConstant.key + "&" + "uuid" + "=" + 2 + "&" + "id" + "=" + 122 + "&" + "l" + "=" + "51.543227,0.0324407" + "&" + "r" + "=" + 10 + "&" + "t" + "=" + 1 + "&" + "keyword" + "=" + res_keyword)
  // .then(function(data){
  //  setMarketNum(data.data.data)
  //  setRestaurantDetail(data.data.data.restaurants)
  //   // alert(JSON.stringify(data.data.data.restaurants[0]))
...
const detail_show=()=>{
  alert(restaurant_detail[0].name)
}
const res_text = navigation.getParam('res_list','nothing sent')
const res_long = res_text.restaurants
    ...
    return (
      ...
      <View style={theme == 'light'?styles.container:Darkstyles.container}>
          ...
          <MapIcon name='map' color={theme == 'light'?'black':'white'} size={25} style={{right:0,position:'absolute',marginRight:20}} onPress={()=>navigation.navigate('mapviewrestaurant',{mapdata:marketnum})}/>
          </View>
          <View>
          <Text style={theme == 'light'?styles.main_text:Darkstyles.main_text}>{res_text.number_rest} restaurants near you</Text>
         <View style={{width:'100%',height:40,flexDirection:'row'}}>
             ...
             <SettingIcon name='player-settings' size={25} color={theme == 'light'?'black':'white'} style={{alignSelf:'center'}} onPress={()=>navigation.navigate('searchgrouprestlist')}/>
             </View>
         </View>
          </View>

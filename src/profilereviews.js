...
const profilereviews=({navigation})=> {
    ...
    return (
      <View style={theme == 'light'?styles.container:Darkstyles.container}>
         ...
         <Image source={theme == 'light' ? require('../img/zabihah.png'):require('../img/Zabihahblack.png')}  style={{alignSelf:'center',padding:10}}/>
          <MenuIcon name='arrowleft' color={theme == 'light'?'black':'white'} size={30} style={{left:0,position:'absolute',marginLeft:13}} onPress={()=>navigation.navigate('detailreview')}/>
          </View>
          <View style={{flexDirection:'row',alignItems:'center',left:20,width:'100%',height:'14%'}}>
              <Avatar rounded source={{uri:mem_data.user_photo}} size="medium" icon={{name:'checkcircle',type:'antdesign',color:'red',size:30,disabled:false}} iconStyle={{marginLeft:30,bottom:10}} onPress={()=>navigation.navigate('publicprofile',{member_id:res_text})}/>
              <View>

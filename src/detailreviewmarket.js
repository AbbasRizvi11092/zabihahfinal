...
const detailreviewmarket=({navigation})=> {
  const [review,setReview] = useState([])
    ...
    useEffect(()=>{
      Axios
      .post(ServiceConstant.data_particular_record + "?" + "key" + "=" + ServiceConstant.key + "&" + "uuid" + "=" + ServiceConstant.uuid + "&" + "id" + "=" + res_id + "&" + "t" + "=" + 2 )
      .then(function(data){
    ...
    return (
     ...
     style={{width:'100%'}}
     renderItem={({item})=>
     <TouchableOpacity style={theme == 'light'?styles.grid_style:Darkstyles.grid_style} onPress={()=>navigation.navigate('profilereviewsmarket',{member_id:item.member_id})}>
     <Row>
         <Col style={{width:60}}>
...
export default detailreviewmarket;

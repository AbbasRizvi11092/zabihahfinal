...
const mapviewrestaurant=({navigation})=> {
  ...
  const map_text = map_main.mosques
  const user_lat = JSON.stringify(map_main.lat_st)
  const user_long = JSON.stringify(map_main.lon_st)
  const res_num = JSON.stringify(map_main.number_mosq)
    return (
      <View style={theme == 'light' ?styles.container:mapstyles.container}>
          <View style={{flexDirection:'row',justifyContent:'center',alignItems:'center'}}>
          <MenuIcon name='arrowleft' color={theme =='light'?"black":'white'} size={30} style={{left:0,position:'absolute',marginLeft:13}} onPress={()=>navigation.navigate('mosquelistview')}/>
          ...
          <View style={{top:30}}>
          <Text style={theme == 'light'?styles.main_text:mapstyles.main_text}>{res_num} mosques & prayers spaces</Text>
         <View style={{width:'100%',height:40,top:20,flexDirection:'row'}}>
             ...
             <SettingIcon name='player-settings' size={25} color={theme =='light'?"black":'white'} style={{alignSelf:'center'}} onPress={()=>navigation.navigate('searchgroupmosquemap')}/>
             </View>
         </View>

...
const mosquelistview=({navigation})=> {
  const [mosque_detail,setMosqueDetail] = useState([])
  const [mosquenum,setMosqueNum] = useState([])
  ...
  useEffect(()=>{
    Axios
    .post(ServiceConstant.record_around_location + "?" + "key" + "=" + ServiceConstant.key + "&" + "uuid" + "=" + ServiceConstant.uuid + "&" + "id" + "=" + ServiceConstant.user_login_data + "&" + "l" + "=" + ServiceConstant.user_lat + "," + ServiceConstant.user_long + "&" + "r" + "=" + 10 + "&" + "t" + "=" + 3 )
    .then(function(data){
     setMosqueNum(data.data.data)
     setMosqueDetail(data.data.data.mosques)
    ...
    return (
      ...
      <View style={theme == 'light'?styles.container:Darkstyles.container}>
          ...
          <MapIcon name='map' color={theme == 'light'?'black':'white'} size={25} style={{right:0,position:'absolute',marginRight:20}} onPress={()=>navigation.navigate('mapviewmosque',{mapdata:mosquenum})}/>
          </View>
          <View>
          <Text style={theme == 'light'?styles.main_text:Darkstyles.main_text}>{JSON.stringify(mosquenum.number_mosq)} mosques & prayers spaces</Text>
         <View style={{width:'100%',height:40,flexDirection:'row'}}>
             ...
             <SettingIcon name='player-settings' size={25} color={theme == 'light'?'black':'white'} style={{alignSelf:'center'}} onPress={()=>navigation.navigate('searchgroupmosquelist')}/>
             </View>
         </View>
          </View>
        {/* <TrendingCard/> */}
        <FlatList data={mosque_detail}
      renderItem={({item})=>
      <Grid style={theme == 'light'?style.main_grid:styleDark.main_grid} onPress={()=>navigation.navigate('mosquetdetail',{mosque_detail:item})}>
      <Row>
          <Col style={{width:110}}>
